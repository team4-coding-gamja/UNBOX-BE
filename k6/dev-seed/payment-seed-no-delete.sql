...
WITH params AS (
    SELECT 200001::int AS seed_start, 210000::int AS seed_end
), seed AS (
    SELECT gs
...
SELECT
    ...
    'CARD' AS method,
    'READY' AS status,
    ('seed_ready_test_success_' || gs) AS payment_key,
    now() AS created_at,
    now() AS ready_at,
    (
...
SELECT
    ...
    NULL AS deleted_by
FROM seed
JOIN p_payment p ON p.payment_key = ('seed_ready_test_success_' || gs)
...
FROM p_payment p
JOIN seed s ON p.payment_key = ('seed_ready_test_success_' || s.gs)
