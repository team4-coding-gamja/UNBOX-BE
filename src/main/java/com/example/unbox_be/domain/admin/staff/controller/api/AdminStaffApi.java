...
@Tag(name = "관리자(Staff)", description = "관리자(스태프) 관리 API")
...
public interface AdminStaffApi {
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 실패", content = @Content),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "권한 없음", content = @Content),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "관리자를 찾을 수 없음", content = @Content)
    })
    @GetMapping("/{adminId}")
    ...
    ApiResponse<Page<AdminStaffListResponseDto>> getAdminStaffPageIncludeDeleted(
            @ParameterObject
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable
    );
}