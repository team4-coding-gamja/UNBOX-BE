package com.example.unbox_be.order.order.service;
...
import com.example.unbox_be.order.order.mapper.OrderMapper;
...
public class OrderServiceImpl implements OrderService {
    ...
    public UUID createOrder(OrderCreateRequestDto requestDto, Long buyerId) {
        ...
        // 2) 판매 입찰 정보 조회
        SellingBidForOrderInfoResponse sellingBidInfo = tradeClientAdapter
    ...
    public OrderDetailResponseDto cancelOrder(UUID orderId, Long userId) {
        ...
        if (previousStatus == OrderStatus.PAYMENT_PENDING) {
            tradeClientAdapter.releaseSellingBid(order.getSellingBidId());
