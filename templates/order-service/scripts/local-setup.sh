...
# Order Service - Local Development Setup Script

echo "🚀 Order Service 로컬 개발 환경 설정을 시작합니다..."
...
if curl -f http://localhost:8084/actuator/health > /dev/null 2>&1; then
    echo "✅ Order Service가 정상적으로 시작되었습니다!"
...
echo "   데이터베이스 접속: docker exec -it unbox-order-postgres psql -U unbox_user -d unbox_order_local"
echo ""