...
# ============================================
# ============================================
# DB Passwords from Secrets Manager
# ============================================
data "aws_secretsmanager_secret" "service_db_passwords" {
  for_each = local.service_config
  
  name = "unbox/prod/${each.key}/db-password"
}

data "aws_secretsmanager_secret_version" "service_db_passwords" {
  for_each = local.service_config
  
  secret_id = data.aws_secretsmanager_secret.service_db_passwords[each.key].id
}
...
module "rds" {
  ...
  service_db_passwords = {
    for service_name in keys(local.service_config) :
    service_name => data.aws_secretsmanager_secret_version.service_db_passwords[service_name].secret_string
  }
}
...
module "codedeploy" {
  ...
  service_names      = ["user", "product", "order", "payment", "trade"]
