...
public enum ErrorCode {
    ...
    SERVICE_UNAVAILABLE(HttpStatus.SERVICE_UNAVAILABLE, "서비스가 일시적으로 이용 불가합니다."),
    RESOURCE_NOT_FOUND(HttpStatus.NOT_FOUND, "요청한 리소스를 찾을 수 없습니다."),
    DATA_INTEGRITY_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "데이터 무결성 오류가 발생했습니다."),
    EXTERNAL_API_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "외부 서비스 통신 중 오류가 발생했습니다."),
    ...
    BID_NOT_MATCHED(HttpStatus.NOT_FOUND, "구매 입찰이 매칭되지 않았습니다."),
    BID_ALREADY_MATCHED(HttpStatus.CONFLICT, "이미 매칭된 구매 입찰입니다."),
    // 리뷰 관련 에러 (Review)
    ...
    PAYMENT_IN_PROGRESS(HttpStatus.CONFLICT, "결제가 이미 승인 처리 중입니다."),
    AMOUNT_MISMATCH(HttpStatus.BAD_REQUEST, "결제 요청 금액이 실제 주문 금액과 일치하지 않습니다."),
    PAYMENT_EXPIRED(HttpStatus.BAD_REQUEST, "결제 유효 시간(10분)이 초과되었습니다."),
    // 정산
