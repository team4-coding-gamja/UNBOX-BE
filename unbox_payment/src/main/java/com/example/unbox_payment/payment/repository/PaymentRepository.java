...
public interface PaymentRepository extends JpaRepository<Payment, UUID> {
    ...
    // ✅ 주문 ID로 결제 내역 조회 (삭제된 데이터 제외) - orderId가 String으로 변경
    @Query("select p from Payment p where p.orderId = :orderId and p.deletedAt is null")
    Optional<Payment> findByOrderIdAndDeletedAtIsNull(@Param("orderId") String orderId);

    // ✅ 주문 ID로 결제 존재 여부 확인 (중복 결제 방지용) - orderId가 String으로 변경
    ...
    // ✅ 완료된 결제 내역 조회 (주문 ID 기준) - orderId가 String으로 변경
    @Query("select p from Payment p where p.orderId = :orderId and p.status = 'DONE' and p.deletedAt is null")
    Optional<Payment> findDonePaymentByOrderId(@Param("orderId") String orderId);
}