...
@Tag(name = "[관리자] 구매입찰 관리", description = "관리자용 구매입찰 관리 API")
@RequestMapping("/api/admin/bids/buying")
public interface AdminBuyingBidApi {

    @Operation(summary = "구매 입찰 목록 조회 (검색 기능 포함)", description = "다양한 조건(상태, 상품명, 브랜드명 등)으로 구매 입찰 목록을 검색하고 페이징 조회합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공", content = @Content(schema = @Schema(implementation = CustomApiResponse.class))),
    ...
    @GetMapping
    CustomApiResponse<Page<AdminBuyingBidListResponseDto>> getBuyingBids(
            @Parameter(description = "검색 조건") @ModelAttribute BuyingBidSearchCondition condition,
            @Parameter(hidden = true) Pageable pageable);

    @Operation(summary = "구매 입찰 삭제", description = "특정 구매 입찰을 삭제(Soft Delete)합니다. 검수자(INSPECTOR)는 삭제할 수 없습니다.")
    @ApiResponses({
    ...
    CustomApiResponse<Void> deleteBuyingBid(
            @Parameter(description = "구매 입찰 UUID", required = true) @PathVariable UUID buyingBidId,
