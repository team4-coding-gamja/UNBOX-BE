...
public interface BuyingBidApi {
        ...
        CustomApiResponse<Void> cancelBuyingBid(
                        @Parameter(description = "구매 입찰 ID", required = true) @PathVariable UUID buyingBidId,
                        @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails);
        ...
        @Operation(summary = "판매자 매칭", description = "로그인한 사용자가 판매자로서 구매 입찰에 매칭합니다.")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "판매자 매칭 성공"),
                        @ApiResponse(responseCode = "400", description = "요청 값 검증 실패 / 매칭 불가 상태", content = @Content),
                        @ApiResponse(responseCode = "401", description = "인증 실패", content = @Content),
                        @ApiResponse(responseCode = "404", description = "구매 입찰을 찾을 수 없음", content = @Content)
        })
        @PostMapping("/{buyingBidId}/match")
        CustomApiResponse<Void> matchBuyingBid(
                        @Parameter(description = "구매 입찰 ID", required = true) @PathVariable UUID buyingBidId,
                        @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails);
